...
function ForgeTab_proddesc120() {
  ...
  const token = typeof window!=='undefined'?localStorage.getItem('forge_token')||'':'';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/ecom/product-description', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify({product,audience})}); const d = await r.json(); setResult(d.result||JSON.stringify(d)); } catch(e:any){setResult('Error: '+e.message);} setLoading(false);, ForgeTab_coverletter121, ForgeTab_ytthumb121, ForgeTab_apiprice121, ForgeTab_onboardemail121, ForgeTab_riskmatrix121 };
...
export function ForgeTab_newsletter127() {
  ...
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📧 Email Newsletter Writer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Write engaging email newsletters that readers actually open — with hooks, value-packed content, and CTAs that convert.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={topic} onChange={e=>setTopic(e.target.value)} placeholder="Newsletter topic / this week's focus (e.g. how to get your first 100 customers, AI tools for designers)..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={audience} onChange={e=>setAudience(e.target.value)} placeholder="Your audience (e.g. indie hackers, B2B SaaS founders, freelance designers)..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!topic||!audience} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Writing...':'Write Newsletter'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}
...
export function ForgeTab_twitterthread127() {
  ...
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🐦 Viral Twitter/X Thread Writer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Write high-engagement Twitter/X threads with a hook that stops the scroll, value-dense content, and a CTA that converts.</p><div style={{display:'grid',gap:'1rem'}}><input value={topic} onChange={e=>setTopic(e.target.value)} placeholder="Thread topic (e.g. 10 lessons from building a $1M SaaS, how to negotiate salary)..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><input value={angle} onChange={e=>setAngle(e.target.value)} placeholder="Your angle or unique insight (e.g. from personal experience, contrarian take, data-driven)..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!topic} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Writing...':'Write Thread'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}
...
export function ForgeTab_salesproposal127() {
  ...
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/sales/proposal', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({client_info:client,solution_description:solution})}); const d = await r.json(); setResult(d.proposal||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📋 Sales Proposal Generator</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Generate a professional, persuasive sales proposal that addresses client pain points, presents your solution, and closes the deal.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={client} onChange={e=>setClient(e.target.value)} placeholder="Client info — company name, industry, size, key pain points, what they told you in discovery..." rows={4} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={solution} onChange={e=>setSolution(e.target.value)} placeholder="Your solution — what you're offering, pricing, timeline, why you're the right fit..." rows={4} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!client||!solution} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Writing...':'Generate Proposal'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}
...
export function ForgeTab_pitchstory127() {
  ...
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/investor/pitch-story', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({startup_description:startup,audience})}); const d = await r.json(); setResult(d.story||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🎯 Pitch Deck Storyteller</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Transform your startup facts into a compelling narrative arc that investors remember long after the meeting ends.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={startup} onChange={e=>setStartup(e.target.value)} placeholder="Your startup — what you do, the problem you solve, traction, team, business model, stage..." rows={5} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={audience} onChange={e=>setAudience(e.target.value)} placeholder="Audience (e.g. seed-stage VCs, angel investors, Series A, corporate partners)..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!startup} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Crafting...':'Craft Pitch Story'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}
...
export function ForgeTab_launchcheck128() {
  ...
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/product/launch-checklist', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,launch_date:launchDate})}); const d = await r.json(); setResult(d.checklist||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🚀 Product Launch Checklist</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Generate a comprehensive pre-launch, launch-day, and post-launch checklist so nothing falls through the cracks.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={product} onChange={e=>setProduct(e.target.value)} placeholder="Your product — what it does, who it's for, how people use it, what stage you're at..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={launchDate} onChange={e=>setLaunchDate(e.target.value)} placeholder="Launch date or timeline (e.g. launching in 2 weeks, launching Q3 2025)..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!product} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Generate Launch Checklist'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_testimonialreq128() {
  const [product, setProduct] = React.useState('');
  const [customerSegment, setCustomerSegment] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/marketing/testimonial-request', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,customer_segment:customerSegment})}); const d = await r.json(); setResult(d.templates||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>⭐ Testimonial Request Email</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Write testimonial request emails that get high response rates — with the exact questions to prompt powerful, usable quotes.</p><div style={{display:'grid',gap:'1rem'}}><input value={product} onChange={e=>setProduct(e.target.value)} placeholder="Your product / service..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><input value={customerSegment} onChange={e=>setCustomerSegment(e.target.value)} placeholder="Customer type (e.g. SaaS founders, e-commerce store owners, freelancers)..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!product} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Writing...':'Generate Request Templates'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}
...
export function ForgeTab_fundraisingemail128() {
  ...
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/investor/fundraising-email', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({startup_description:startup,investor_type:investorType})}); const d = await r.json(); setResult(d.emails||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>💰 Fundraising Pitch Email</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Write cold investor emails that get replies — personalized, concise, with a hook that makes them want to learn more.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={startup} onChange={e=>setStartup(e.target.value)} placeholder="Your startup — what you do, traction, stage, raise amount, what you'll use the money for..." rows={4} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={investorType} onChange={e=>setInvestorType(e.target.value)} placeholder="Investor type (e.g. seed VC, angel, strategic corporate, family office)..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!startup} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Writing...':'Write Pitch Emails'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_offboardingsurvey128() {
  const [product, setProduct] = React.useState('');
  const [churnReason, setChurnReason] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/cx/offboarding-survey', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,suspected_churn_reason:churnReason})}); const d = await r.json(); setResult(d.survey||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📊 User Offboarding Survey</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Design a cancellation/offboarding survey that surfaces real reasons users leave — and saves some of them in the process.</p><div style={{display:'grid',gap:'1rem'}}><input value={product} onChange={e=>setProduct(e.target.value)} placeholder="Your product / service..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><input value={churnReason} onChange={e=>setChurnReason(e.target.value)} placeholder="Most common churn reason you hear (e.g. too expensive, missing features, not using it)..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!product} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Design Offboarding Survey'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}
...
export function ForgeTab_ideavalidator129() {
  ...
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/startup/idea-validator', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({idea,target_market:market})}); const d = await r.json(); setResult(d.validation||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>💡 Startup Idea Validator</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Stress-test your startup idea like a seasoned VC — get an honest assessment of market size, competition, moat, and go-to-market viability.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={idea} onChange={e=>setIdea(e.target.value)} placeholder="Your startup idea — what it does, how it works, how you make money..." rows={4} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={market} onChange={e=>setMarket(e.target.value)} placeholder="Target market (who buys this, how many are there, what do they currently do instead)..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!idea} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Validating...':'Validate Idea'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}
...
export function ForgeTab_linkedindm129() {
  ...
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/social/linkedin-dm', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({target_person:targetPerson,purpose})}); const d = await r.json(); setResult(d.messages||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>💼 Cold LinkedIn DM Writer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Write cold LinkedIn DMs that get replies — no pitch slapping, no "I'd love to connect", just real human messages that open conversations.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={targetPerson} onChange={e=>setTargetPerson(e.target.value)} placeholder="Who you're messaging — their role, company, something specific from their profile (recent post, shared connection, career move)..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={purpose} onChange={e=>setPurpose(e.target.value)} placeholder="Your goal (e.g. intro meeting, partnership conversation, ask for advice, recruit, sell something)..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!targetPerson||!purpose} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Writing...':'Write LinkedIn DM'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}
...
export function ForgeTab_pitchvideo129() {
  ...
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/content/pitch-video-script', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,duration})}); const d = await r.json(); setResult(d.script||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🎬 Pitch Video Script</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Write a compelling product pitch video script — the kind that gets watched to the end and makes viewers want to sign up immediately.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={product} onChange={e=>setProduct(e.target.value)} placeholder="Your product — what it does, who it's for, key features, what makes it different..." rows={4} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={duration} onChange={e=>setDuration(e.target.value)} placeholder="Target video length (e.g. 60 seconds, 2 minutes, 5 minutes)..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!product} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Writing...':'Write Video Script'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_saasonboarding129() {
  const [product, setProduct] = React.useState('');
  const [keyAction, setKeyAction] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/saas/onboarding-sequence', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,key_activation_action:keyAction})}); const d = await r.json(); setResult(d.sequence||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>✉️ SaaS Onboarding Email Sequence</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Design the onboarding email sequence that turns trial signups into activated users — and activated users into paying customers.</p><div style={{display:'grid',gap:'1rem'}}><input value={product} onChange={e=>setProduct(e.target.value)} placeholder="Your SaaS product..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><input value={keyAction} onChange={e=>setKeyAction(e.target.value)} placeholder="Key activation action — the single moment users 'get it' (e.g. first project created, first integration connected, first report generated)..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!product||!keyAction} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Onboarding Sequence'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}
...
export function ForgeTab_feedbackanalyzer129() {
  ...
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/product/feedback-analyzer', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({feedback,product_context:productContext})}); const d = await r.json(); setResult(d.analysis||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🔍 Product Feedback Analyzer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Paste raw user feedback and get it transformed into prioritized product insights, feature requests, and actionable next steps.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={feedback} onChange={e=>setFeedback(e.target.value)} placeholder="Paste user feedback here — support tickets, app store reviews, survey responses, interview notes, tweets, anything..." rows={6} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={productContext} onChange={e=>setProductContext(e.target.value)} placeholder="Product context (what stage you're at, what you're focused on building next, top priorities)..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!feedback} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Analyzing...':'Analyze Feedback'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}
...
export function ForgeTab_nichefinder130() {
  ...
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/business/niche-finder', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({skills,interests})}); const d = await r.json(); setResult(d.niches||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🎯 Niche Market Finder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Discover underserved niche markets that match your skills and interests — with monetization paths and competition analysis.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={skills} onChange={e=>setSkills(e.target.value)} placeholder="Your skills, experience, and expertise (e.g. 10 years in HR, know Python, built SaaS before, good at video editing)..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={interests} onChange={e=>setInterests(e.target.value)} placeholder="Your interests, hobbies, and passions (industries you find interesting, problems you care about, communities you're part of)..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!skills} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Finding...':'Find My Niche'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}
...
export function ForgeTab_ytdescription130() {
  ...
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/content/youtube-description', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({video_title:videoTitle,video_content:videoContent})}); const d = await r.json(); setResult(d.description||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📺 YouTube Description Writer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Write SEO-optimized YouTube descriptions that rank in search, drive clicks, and convert viewers into subscribers.</p><div style={{display:'grid',gap:'1rem'}}><input value={videoTitle} onChange={e=>setVideoTitle(e.target.value)} placeholder="Video title..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><textarea value={videoContent} onChange={e=>setVideoContent(e.target.value)} placeholder="What the video covers — main topics, key points, who it's for, any links to include..." rows={4} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!videoTitle||!videoContent} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Writing...':'Write Description'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}
...
export function ForgeTab_faqgenerator130() {
  ...
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>❓ FAQ Generator</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Generate comprehensive FAQ pages that pre-answer objections, reduce support tickets, and improve SEO rankings.</p><div style={{display:'grid',gap:'1rem'}}><input value={product} onChange={e=>setProduct(e.target.value)} placeholder="Your product / service / topic..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><textarea value={context} onChange={e=>setContext(e.target.value)} placeholder="Additional context — pricing, features, common complaints, target audience, what makes you different..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!product} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Generating...':'Generate FAQs'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}
...
export function ForgeTab_pricingobjection130() {
  ...
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/sales/pricing-objection-handler', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,price_point:pricePoint})}); const d = await r.json(); setResult(d.responses||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>💬 Pricing Objection Handler</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Get ready responses for every pricing objection — "too expensive", "can I get a discount?", "competitor X is cheaper" — and more.</p><div style={{display:'grid',gap:'1rem'}}><input value={product} onChange={e=>setProduct(e.target.value)} placeholder="Your product / service..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><input value={pricePoint} onChange={e=>setPricePoint(e.target.value)} placeholder="Your price point / pricing model (e.g. $299/mo, $5,000 one-time, $97/seat/month)..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!product||!pricePoint} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Objection Handlers'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}
...
export function ForgeTab_execsummary130() {
  ...
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/writing/executive-summary', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({document_content:document,audience})}); const d = await r.json(); setResult(d.summary||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📄 Executive Summary Writer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Write a crisp executive summary of any document, report, or proposal — the kind busy leaders actually read.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={document} onChange={e=>setDocument(e.target.value)} placeholder="Paste your document, report, proposal, or business plan here (or describe what it covers and its key findings)..." rows={6} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={audience} onChange={e=>setAudience(e.target.value)} placeholder="Target audience for the summary (e.g. board of directors, investors, VP of Sales, department heads)..." style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!document} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Writing...':'Write Executive Summary'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

ForgeTab_pressrel126, ForgeTab_pricingpage126, ForgeTab_techdoc126, ForgeTab_csplaybook126, ForgeTab_influencer126, ForgeTab_brandvoice127, ForgeTab_newsletter127, ForgeTab_twitterthread127, ForgeTab_salesproposal127, ForgeTab_pitchstory127, ForgeTab_launchcheck128, ForgeTab_testimonialreq128, ForgeTab_fundraisingemail128, ForgeTab_offboardingsurvey128, ForgeTab_agentprompt128, ForgeTab_ideavalidator129, ForgeTab_linkedindm129, ForgeTab_pitchvideo129, ForgeTab_saasonboarding129, ForgeTab_feedbackanalyzer129, ForgeTab_nichefinder130, ForgeTab_ytdescription130, ForgeTab_faqgenerator130, ForgeTab_pricingobjection130, ForgeTab_execsummary130 };
