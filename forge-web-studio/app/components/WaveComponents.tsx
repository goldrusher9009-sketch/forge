...
export function ForgeTab_strategicplan181() {
  const [company, setCompany] = React.useState('');
  const [horizon, setHorizon] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/product/strategic-plan', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({company,horizon})}); const d = await r.json(); setResult(d.plan||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🗺️ Strategic Plan Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build a focused strategic plan — not a document that sits in a drawer, but a clear direction with the priorities and tradeoffs that make real strategy actionable.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={company} onChange={e=>setCompany(e.target.value)} placeholder="Company: what you do, current situation (strengths, weaknesses, threats, opportunities), what you are trying to achieve..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={horizon} onChange={e=>setHorizon(e.target.value)} placeholder="Planning horizon (e.g. 1-year, 3-year, next 18 months to Series A)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!company||!horizon} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Strategic Plan'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_apidesign181() {
  const [service, setService] = React.useState('');
  const [usecase, setUsecase] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/dev/api-design', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({service,usecase})}); const d = await r.json(); setResult(d.design||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🔌 API Design Advisor</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Design a clean API that developers love — REST or GraphQL, resource modeling, naming conventions, versioning strategy, and the patterns that make it intuitive to use.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={service} onChange={e=>setService(e.target.value)} placeholder="Service: what it does, the entities and relationships, expected usage patterns, who will consume this API..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={usecase} onChange={e=>setUsecase(e.target.value)} placeholder="Primary use case (e.g. public API for third-party developers, internal microservice, mobile app backend)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!service||!usecase} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Designing...':'Design API'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_churnprevention181() {
  const [product, setProduct] = React.useState('');
  const [signals, setSignals] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/product/churn-prevention', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,signals})}); const d = await r.json(); setResult(d.plan||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🛟 Churn Prevention Playbook</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build a churn prevention system — the early warning signals, intervention triggers, and playbooks that keep customers before they decide to leave.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={product} onChange={e=>setProduct(e.target.value)} placeholder="Product: what it does, customer segment, typical contract length, current churn rate, why customers have churned..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={signals} onChange={e=>setSignals(e.target.value)} placeholder="Leading signals you can track (e.g. login frequency, feature usage, support tickets, NPS score, payment failures)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!product||!signals} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Playbook'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_recruitingscript181() {
  const [role, setRole] = React.useState('');
  const [company, setCompany] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/hr/recruiting-script', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({role,company})}); const d = await r.json(); setResult(d.script||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📞 Recruiting Outreach Script</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Write recruiting outreach that gets responses — personalized, specific, and compelling enough that passive candidates actually reply.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={role} onChange={e=>setRole(e.target.value)} placeholder="Role: title, what the person will do, what makes this opportunity interesting, key requirements..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={company} onChange={e=>setCompany(e.target.value)} placeholder="Company: what you do, stage, why someone would want to join, culture, what you offer..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!role||!company} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Writing...':'Write Script'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_decknarrative181() {
  const [company, setCompany] = React.useState('');
  const [audience, setAudience] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/investor/deck-narrative', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({company,audience})}); const d = await r.json(); setResult(d.narrative||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📊 Pitch Deck Narrative</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build the narrative arc for your pitch deck — the story that makes investors feel the inevitability of your company before you get to the numbers.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={company} onChange={e=>setCompany(e.target.value)} placeholder="Company: what you do, the problem, your insight, traction, team, where you are going, why you will win..." rows={4} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={audience} onChange={e=>setAudience(e.target.value)} placeholder="Audience (e.g. seed VCs, Series A, strategic investors, family office, corporate VC)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!company||!audience} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Narrative'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_retentionprogram182() {
  const [product, setProduct] = React.useState('');
  const [segment, setSegment] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/marketing/retention-program', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,segment})}); const d = await r.json(); setResult(d.program||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🔄 Retention Program Designer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Design a customer retention program — the milestones, habits, and touchpoints that deepen the relationship and make leaving costly.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={product} onChange={e=>setProduct(e.target.value)} placeholder="Product: what it does, how value compounds over time, current retention rate, best retained customers and what they do differently..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={segment} onChange={e=>setSegment(e.target.value)} placeholder="Target segment (e.g. 90-day customers, power users, at-risk accounts, annual subscribers)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!product||!segment} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Designing...':'Design Program'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_okcrsystem182() {
  const [company, setCompany] = React.useState('');
  const [period, setPeriod] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/productivity/okr-system', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({company,period})}); const d = await r.json(); setResult(d.okrs||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🎯 OKR System Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build an OKR system that drives focus — well-formed objectives, measurable key results, and the process to make OKRs something people actually use.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={company} onChange={e=>setCompany(e.target.value)} placeholder="Company: what you do, biggest priorities right now, what you are trying to accomplish, current challenges..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={period} onChange={e=>setPeriod(e.target.value)} placeholder="Period (e.g. Q3 2025, H2 2025, annual OKRs for 2026)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!company||!period} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build OKRs'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_datamodel182() {
  const [product, setProduct] = React.useState('');
  const [requirements, setRequirements] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/dev/data-model', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,requirements})}); const d = await r.json(); setResult(d.model||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🗄️ Data Model Designer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Design a data model that scales — entities, relationships, key decisions, and the tradeoffs that will matter when you have 100x the data you have now.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={product} onChange={e=>setProduct(e.target.value)} placeholder="Product: what it does, the core entities, how data flows, key queries you need to run, expected scale..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={requirements} onChange={e=>setRequirements(e.target.value)} placeholder="Requirements: read vs. write ratio, latency needs, consistency requirements, multi-tenancy, compliance..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!product||!requirements} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Designing...':'Design Data Model'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_csmplaybook182() {
  const [product, setProduct] = React.useState('');
  const [segment, setSegment] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/productivity/csm-playbook', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,segment})}); const d = await r.json(); setResult(d.playbook||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🤝 CSM Playbook Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build a Customer Success playbook — the touchpoints, QBR templates, expansion plays, and escalation logic that turns a CSM into a revenue driver.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={product} onChange={e=>setProduct(e.target.value)} placeholder="Product: what it does, how customers get value, contract length and value, key success metrics for customers..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={segment} onChange={e=>setSegment(e.target.value)} placeholder="Customer segment (e.g. SMB self-serve, mid-market, enterprise, accounts over $50K ARR)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!product||!segment} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Playbook'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_marketingops182() {
  const [stack, setStack] = React.useState('');
  const [goal, setGoal] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/marketing/marketing-ops-audit', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({stack,goal})}); const d = await r.json(); setResult(d.audit||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>⚙️ Marketing Ops Audit</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Audit your marketing operations — the tools, processes, and data flows that are slowing you down or costing you conversions without you knowing it.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={stack} onChange={e=>setStack(e.target.value)} placeholder="Marketing stack: tools you use (CRM, MAP, analytics, ads, email, etc.), how they connect, biggest friction points..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={goal} onChange={e=>setGoal(e.target.value)} placeholder="Goal (e.g. improve lead routing speed, reduce CAC, better attribution, scale email volume)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!stack||!goal} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Auditing...':'Run Audit'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_saasmetrics183() {
  const [company, setCompany] = React.useState('');
  const [stage, setStage] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/finance/saas-metrics', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({company,stage})}); const d = await r.json(); setResult(d.metrics||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>💹 SaaS Metrics Calculator</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Calculate and benchmark your SaaS metrics — ARR, NRR, CAC, LTV, payback period, and the ratios that tell you whether your unit economics are actually working.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={company} onChange={e=>setCompany(e.target.value)} placeholder="Company data: MRR, growth rate, churn rate, CAC by channel, ACV, average contract length, expansion revenue..." rows={4} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={stage} onChange={e=>setStage(e.target.value)} placeholder="Stage (e.g. $500K ARR seed stage, $5M ARR Series A, $20M ARR growth)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!company||!stage} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Calculating...':'Calculate Metrics'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_communicationplan183() {
  const [change, setChange] = React.useState('');
  const [stakeholders, setStakeholders] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/productivity/communication-plan', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({change,stakeholders})}); const d = await r.json(); setResult(d.plan||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📢 Communication Plan Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build a communication plan for a change — who hears what, in what order, through which channel, with what message tailored to their concerns.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={change} onChange={e=>setChange(e.target.value)} placeholder="The change: what is happening, why, what the impact is, timeline, what questions you expect..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={stakeholders} onChange={e=>setStakeholders(e.target.value)} placeholder="Stakeholder groups (e.g. employees, customers, investors, press, partners, regulators)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!change||!stakeholders} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Plan'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_uxresearch183() {
  const [product, setProduct] = React.useState('');
  const [question, setQuestion] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/product/ux-research-plan', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,question})}); const d = await r.json(); setResult(d.plan||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🔬 UX Research Plan</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Design a UX research study — the right method for your question, recruitment criteria, interview guide, and how to turn findings into product decisions.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={product} onChange={e=>setProduct(e.target.value)} placeholder="Product: what it does, who uses it, what you are trying to improve or understand..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={question} onChange={e=>setQuestion(e.target.value)} placeholder="Research question (e.g. why do users not complete onboarding, what motivates premium upgrades, how do power users use X)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!product||!question} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Designing...':'Design Research Plan'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_contractnegotiate183() {
  const [contract, setContract] = React.useState('');
  const [position, setPosition] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/sales/contract-negotiation', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({contract,position})}); const d = await r.json(); setResult(d.strategy||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>⚖️ Contract Negotiation Coach</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Get a negotiation strategy for your contract — what to push on, what to concede, how to frame each ask, and the walk-away points that protect what matters.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={contract} onChange={e=>setContract(e.target.value)} placeholder="Contract terms: key clauses in dispute, deal size, what the other side wants, deadline pressure, your alternatives..." rows={4} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={position} onChange={e=>setPosition(e.target.value)} placeholder="Your position (e.g. vendor selling to enterprise, startup accepting terms, both sides trying to close by month-end)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!contract||!position} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Strategizing...':'Get Strategy'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_devcodereview183() {
  const [code, setCode] = React.useState('');
  const [context, setContext] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/dev/code-review-guide', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({code,context})}); const d = await r.json(); setResult(d.review||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🔍 Code Review Guide</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Get a structured code review — not just style comments, but the bugs, edge cases, performance issues, and security concerns that matter most in production.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={code} onChange={e=>setCode(e.target.value)} placeholder="Code or PR description: paste the code, describe what the PR does, what it changes, what edge cases it handles..." rows={5} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical',fontFamily:'monospace'}} /><input value={context} onChange={e=>setContext(e.target.value)} placeholder="Context (language, framework, production environment, performance requirements, security sensitivity)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!code||!context} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Reviewing...':'Review Code'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_productpricingmodel184() {
  const [product, setProduct] = React.useState('');
  const [customers, setCustomers] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/product/pricing-model', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,customers})}); const d = await r.json(); setResult(d.model||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🧮 Pricing Model Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build a pricing model from first principles — the right metric, the right structure, and the price points that capture value without leaving money on the table or killing conversions.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={product} onChange={e=>setProduct(e.target.value)} placeholder="Product: what it does, value it delivers, how value scales with usage, current pricing if any, conversion rates..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={customers} onChange={e=>setCustomers(e.target.value)} placeholder="Customers: segments, willingness to pay signals, what they compare your price to, deal sizes you have seen..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!product||!customers} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Pricing Model'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_hrofferletter184() {
  const [candidate, setCandidate] = React.useState('');
  const [offer, setOffer] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/hr/offer-letter', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({candidate,offer})}); const d = await r.json(); setResult(d.letter||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📝 Offer Letter Writer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Write an offer letter that makes candidates excited to say yes — warm, professional, clear on the terms, and compelling about why this is the right move for them.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={candidate} onChange={e=>setCandidate(e.target.value)} placeholder="Candidate: name, role, what impressed you most about them, what you are excited about them bringing to the team..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={offer} onChange={e=>setOffer(e.target.value)} placeholder="Offer details: base salary, equity (options or RSUs, cliff, vesting), bonus, benefits, start date, reporting structure, any special terms..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!candidate||!offer} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Writing...':'Write Offer Letter'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_productmvp184() {
  const [idea, setIdea] = React.useState('');
  const [constraints, setConstraints] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/product/mvp-scope', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({idea,constraints})}); const d = await r.json(); setResult(d.scope||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🚀 MVP Scope Definer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Define your MVP scope — ruthlessly cut to the minimum that tests your most critical assumptions and delivers enough value for real feedback.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={idea} onChange={e=>setIdea(e.target.value)} placeholder="Product idea: what you are building, who it is for, the core hypothesis you need to validate, features you have in mind..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={constraints} onChange={e=>setConstraints(e.target.value)} placeholder="Constraints (e.g. 4-week timeline, 2 engineers, $10K budget, must launch before conference)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!idea||!constraints} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Scoping...':'Define MVP'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_salescomp184() {
  const [role, setRole] = React.useState('');
  const [model, setModel] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/hr/sales-comp-design', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({role,model})}); const d = await r.json(); setResult(d.comp||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>💰 Sales Comp Plan Designer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Design a sales compensation plan that drives the right behaviors — attainable quotas, motivating accelerators, and a structure that retains your best reps.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={role} onChange={e=>setRole(e.target.value)} placeholder="Role: title, type of sales (inbound, outbound, channel, enterprise), deal size, sales cycle length, expected quota attainment..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={model} onChange={e=>setModel(e.target.value)} placeholder="Business model: what you sell, ARR target per rep, current OTE range if any, what behaviors you need to drive..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!role||!model} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Designing...':'Design Comp Plan'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_devmigration184() {
  const [migration, setMigration] = React.useState('');
  const [system, setSystem] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/dev/migration-plan', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({migration,system})}); const d = await r.json(); setResult(d.plan||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🔀 Migration Plan Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build a migration plan that does not blow up production — phased approach, rollback strategy, validation checkpoints, and the order of operations that minimizes risk.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={migration} onChange={e=>setMigration(e.target.value)} placeholder="Migration: what you are moving (database, infrastructure, service, framework), from what to what, why..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={system} onChange={e=>setSystem(e.target.value)} placeholder="System: current scale, uptime requirements, team size, traffic patterns, data sensitivity, acceptable maintenance window..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!migration||!system} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Planning...':'Build Migration Plan'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_investornarrative185() {
  const [company, setCompany] = React.useState('');
  const [stage, setStage] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/investor/narrative-builder', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({company,stage})}); const d = await r.json(); setResult(d.narrative||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📖 Investor Narrative Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build the narrative arc that makes investors lean in — the story behind the business that connects market pain, your insight, traction, and the vision in a way that feels inevitable.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={company} onChange={e=>setCompany(e.target.value)} placeholder="Company: what you do, who you serve, key traction, what makes you different, founding story if compelling..." rows={4} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={stage} onChange={e=>setStage(e.target.value)} placeholder="Stage and round (e.g. Series A raising $8M, pre-revenue seed, $3M ARR Series B)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!company||!stage} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Narrative'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_teamretro185() {
  const [sprint, setSprint] = React.useState('');
  const [team, setTeam] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/productivity/team-retro', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({sprint,team})}); const d = await r.json(); setResult(d.retro||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🔄 Team Retrospective Facilitator</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Run a productive retrospective — structured format, the right questions, and a concrete action plan that the team will actually follow through on.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={sprint} onChange={e=>setSprint(e.target.value)} placeholder="Sprint/period: what happened, wins, misses, blockers, key events, anything notable..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={team} onChange={e=>setTeam(e.target.value)} placeholder="Team context (size, type — eng, product, sales, what the retro format should be)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!sprint||!team} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Facilitating...':'Run Retro'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_marketingaudit185() {
  const [business, setBusiness] = React.useState('');
  const [channels, setChannels] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/marketing/channel-audit', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({business,channels})}); const d = await r.json(); setResult(d.audit||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📊 Marketing Channel Audit</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Audit your marketing channels — which are performing, which are wasting budget, and where the best opportunities are hiding given your stage and resources.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={business} onChange={e=>setBusiness(e.target.value)} placeholder="Business: what you sell, ICP, revenue stage, team size, total marketing budget..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={channels} onChange={e=>setChannels(e.target.value)} placeholder="Current channels: list each with spend, leads/month, conversion rate, CAC if known (e.g. Google Ads: $5K/mo, 50 leads, 10% close, $1K CAC)" rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!business||!channels} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Auditing...':'Run Audit'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_engonboarding185() {
  const [role, setRole] = React.useState('');
  const [stack, setStack] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/dev/engineer-onboarding', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({role,stack})}); const d = await r.json(); setResult(d.plan||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>👨‍💻 Engineer Onboarding Plan</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build an onboarding plan that gets engineers to their first meaningful commit fast — structured ramp, the right learning order, and early wins that build confidence.</p><div style={{display:'grid',gap:'1rem'}}><input value={role} onChange={e=>setRole(e.target.value)} placeholder="Role (e.g. senior backend engineer, junior fullstack, staff infrastructure)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><textarea value={stack} onChange={e=>setStack(e.target.value)} placeholder="Tech stack and team context: languages, frameworks, codebase size, team process (sprints, code review, deploys), what good looks like at 30/60/90 days..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!role||!stack} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Onboarding Plan'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_productlaunchplan185() {
  const [product, setProduct] = React.useState('');
  const [timeline, setTimeline] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/product/launch-plan', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,timeline})}); const d = await r.json(); setResult(d.plan||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🎯 Product Launch Plan</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build a complete product launch plan — go-to-market sequencing, channel strategy, launch messaging, success metrics, and the checklist that keeps everyone aligned.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={product} onChange={e=>setProduct(e.target.value)} placeholder="Product: what you are launching, target audience, key value prop, existing user base (if any), pricing..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={timeline} onChange={e=>setTimeline(e.target.value)} placeholder="Timeline (e.g. launching in 6 weeks, 3-month runway to hit X target, soft launch then full)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!product||!timeline} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Planning...':'Build Launch Plan'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_salesscript186() {
  const [product, setProduct] = React.useState('');
  const [persona, setPersona] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/sales/call-script', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,persona})}); const d = await r.json(); setResult(d.script||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📞 Sales Call Script</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build a sales call script that opens conversations, asks the right discovery questions, handles objections, and moves deals forward without sounding scripted.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={product} onChange={e=>setProduct(e.target.value)} placeholder="Product: what it does, price point, key value props, common objections you hear, typical sales cycle..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={persona} onChange={e=>setPersona(e.target.value)} placeholder="Prospect persona (e.g. VP Sales at 50-200 person SaaS, CTO at mid-market company, SMB owner)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!product||!persona} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Writing...':'Build Script'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_boardupdate186() {
  const [period, setPeriod] = React.useState('');
  const [metrics, setMetrics] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/investor/board-update', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({period,metrics})}); const d = await r.json(); setResult(d.update||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📋 Board Update Writer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Write a board update that builds confidence — transparent on challenges, clear on what you are doing about them, and focused on what the board needs to do to help.</p><div style={{display:'grid',gap:'1rem'}}><input value={period} onChange={e=>setPeriod(e.target.value)} placeholder="Period (e.g. Q2 2025, May 2025, H1 2025)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><textarea value={metrics} onChange={e=>setMetrics(e.target.value)} placeholder="Metrics and narrative: ARR/revenue, growth rate, key wins, key misses, team changes, fundraising status, top risks, asks from the board..." rows={4} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!period||!metrics} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Writing...':'Write Board Update'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_hrperformance186() {
  const [employee, setEmployee] = React.useState('');
  const [context, setContext] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/hr/performance-review', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({employee,context})}); const d = await r.json(); setResult(d.review||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>⭐ Performance Review Writer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Write a performance review that is honest, specific, and actionable — the kind that actually helps people grow rather than a box-checking exercise.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={employee} onChange={e=>setEmployee(e.target.value)} placeholder="Employee: role, tenure, key accomplishments this period, areas of strength, where they struggled, projects they led..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={context} onChange={e=>setContext(e.target.value)} placeholder="Context: overall rating, calibration position, compensation decision, is this a top performer, middle, or underperformer, growth trajectory..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!employee||!context} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Writing...':'Write Review'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_devloadtest186() {
  const [system, setSystem] = React.useState('');
  const [targets, setTargets] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/dev/load-test-plan', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({system,targets})}); const d = await r.json(); setResult(d.plan||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>⚡ Load Test Plan Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Design a load test that actually finds your breaking point before your users do — realistic traffic patterns, the right tool, and success criteria that matter.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={system} onChange={e=>setSystem(e.target.value)} placeholder="System: what you are testing, architecture (API, database, queue), current traffic, expected peak, any known bottlenecks..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={targets} onChange={e=>setTargets(e.target.value)} placeholder="Performance targets (e.g. p99 < 500ms, sustain 1000 RPS, 10K concurrent users, 99.9% availability)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!system||!targets} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Designing...':'Build Load Test Plan'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_productanalytics186() {
  const [product, setProduct] = React.useState('');
  const [goals, setGoals] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/product/analytics-strategy', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,goals})}); const d = await r.json(); setResult(d.strategy||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📈 Product Analytics Strategy</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Design your product analytics strategy — the events to track, the funnels to monitor, and the dashboards that tell you what users are actually doing versus what you think they are doing.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={product} onChange={e=>setProduct(e.target.value)} placeholder="Product: what it does, key user flows, current analytics setup (if any), what decisions you are trying to make with data..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={goals} onChange={e=>setGoals(e.target.value)} placeholder="Business goals (e.g. improve activation, reduce churn, increase expansion revenue, find conversion drop-offs)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!product||!goals} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Designing...':'Design Analytics Strategy'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_pricingstrategy187() {
  const [market, setMarket] = React.useState('');
  const [competitors, setCompetitors] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/marketing/pricing-strategy', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({market,competitors})}); const d = await r.json(); setResult(d.strategy||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>💲 Pricing Strategy Advisor</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Develop a pricing strategy grounded in market reality — competitor positioning, buyer psychology, and the price points that maximize both conversions and revenue.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={market} onChange={e=>setMarket(e.target.value)} placeholder="Market: what you sell, target buyers, deal sizes you see, willingness to pay signals, what price objections you hear..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={competitors} onChange={e=>setCompetitors(e.target.value)} placeholder="Competitors: who they are, their pricing (public or estimated), what tier of the market they serve, where you differentiate..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!market||!competitors} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Analyzing...':'Build Strategy'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_culturecode187() {
  const [values, setValues] = React.useState('');
  const [behaviors, setBehaviors] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/hr/culture-code', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({values,behaviors})}); const d = await r.json(); setResult(d.culture||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🎯 Culture Code Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Write your culture code — the values that actually drive decisions, the behaviors that define membership, and the norms that make your team different from every other team.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={values} onChange={e=>setValues(e.target.value)} placeholder="Your values: what you genuinely believe (not aspirational platitudes), what you have seen your best people do that made you proud, what gets you fired..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={behaviors} onChange={e=>setBehaviors(e.target.value)} placeholder="Observed behaviors: how do decisions actually get made, how does the team communicate, what makes someone successful here vs. not..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!values||!behaviors} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Culture Code'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_devdebugguide187() {
  const [issue, setIssue] = React.useState('');
  const [environment, setEnvironment] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/dev/debug-guide', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({issue,environment})}); const d = await r.json(); setResult(d.guide||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🐛 Debug Guide Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Get a systematic debugging guide — the right order to investigate, what each symptom means, and the tools to reach for at each step of the investigation.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={issue} onChange={e=>setIssue(e.target.value)} placeholder="Issue: what is broken, symptoms observed, error messages or logs, when it started, what changed before it broke..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical',fontFamily:'monospace'}} /><input value={environment} onChange={e=>setEnvironment(e.target.value)} placeholder="Environment (language, framework, infrastructure, production vs. local, observable vs. flaky)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!issue||!environment} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Analyzing...':'Build Debug Guide'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_productdiscovery187() {
  const [problem, setProblem] = React.useState('');
  const [users, setUsers] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/product/discovery-sprint', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({problem,users})}); const d = await r.json(); setResult(d.sprint||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🔎 Product Discovery Sprint</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Plan a product discovery sprint — the right activities, the right questions, and the outputs that give your team confidence to build versus confidence to pivot.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={problem} onChange={e=>setProblem(e.target.value)} placeholder="Problem space: what you think users need, current evidence, assumptions you are most uncertain about, what is at stake if you are wrong..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={users} onChange={e=>setUsers(e.target.value)} placeholder="Target users (who to recruit, how many, their profile)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!problem||!users} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Planning...':'Plan Discovery Sprint'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_salesqbr187() {
  const [quarter, setQuarter] = React.useState('');
  const [data, setData] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/sales/qbr-prep', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({quarter,data})}); const d = await r.json(); setResult(d.qbr||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📊 QBR Prep Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build a Quarterly Business Review that keeps customers — honest performance analysis, value demonstration, and a forward plan that makes renewal feel obvious.</p><div style={{display:'grid',gap:'1rem'}}><input value={quarter} onChange={e=>setQuarter(e.target.value)} placeholder="Quarter (e.g. Q2 2025)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><textarea value={data} onChange={e=>setData(e.target.value)} placeholder="Customer data: usage metrics, goals from last QBR, wins delivered, challenges faced, renewal date, expansion opportunities, stakeholders attending..." rows={4} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!quarter||!data} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build QBR'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_designsystem188() {
  const [product, setProduct] = React.useState('');
  const [current, setCurrent] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/product/design-system-guide', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,current})}); const d = await r.json(); setResult(d.guide||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🎨 Design System Guide</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build or evolve your design system — the component library, design tokens, and documentation standards that let your team ship consistent UI without constant design review.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={product} onChange={e=>setProduct(e.target.value)} placeholder="Product: type of app, platform (web, mobile, both), team size, how fast you ship, primary use cases..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={current} onChange={e=>setCurrent(e.target.value)} placeholder="Current state: existing component library or not, design tool (Figma, Sketch), pain points, inconsistencies, what is slowing your team down..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!product||!current} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Design System Guide'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_hiringplan188() {
  const [stage, setStage] = React.useState('');
  const [goals, setGoals] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/hr/hiring-plan', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({stage,goals})}); const d = await r.json(); setResult(d.plan||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>👥 Hiring Plan Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build a hiring plan that is actually achievable — the right roles in the right order, realistic timelines, budget, and the sequencing that lets each hire unlock the next.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={stage} onChange={e=>setStage(e.target.value)} placeholder="Company stage: current headcount, runway, funding stage, what you are building in the next 12 months, current team gaps..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={goals} onChange={e=>setGoals(e.target.value)} placeholder="Goals: what you need to achieve this year (revenue, product milestones, markets), roles you are considering, budget range for hiring..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!stage||!goals} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Planning...':'Build Hiring Plan'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_marketingfunnel188() {
  const [business, setBusiness] = React.useState('');
  const [stage, setStage] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/marketing/funnel-design', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({business,stage})}); const d = await r.json(); setResult(d.funnel||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🔽 Marketing Funnel Designer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Design your full marketing funnel — top to bottom, with the right content and touchpoints at each stage, conversion benchmarks, and the levers that move people through fastest.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={business} onChange={e=>setBusiness(e.target.value)} placeholder="Business: what you sell, who buys it, average deal size, current lead volume, conversion rates if known, sales cycle length..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={stage} onChange={e=>setStage(e.target.value)} placeholder="Buying stage complexity (e.g. high-consideration enterprise, self-serve PLG, SMB with 2-week sales cycle)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!business||!stage} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Designing...':'Design Funnel'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_devcicd188() {
  const [stack, setStack] = React.useState('');
  const [requirements, setRequirements] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/dev/cicd-pipeline', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({stack,requirements})}); const d = await r.json(); setResult(d.pipeline||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>⚙️ CI/CD Pipeline Designer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Design your CI/CD pipeline — the right stages, the right tools, and the configuration that gives your team fast feedback without flaky builds blocking deployments.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={stack} onChange={e=>setStack(e.target.value)} placeholder="Tech stack: language, framework, containerized or not, cloud provider, current deployment method, test suite size..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={requirements} onChange={e=>setRequirements(e.target.value)} placeholder="Requirements: deploy frequency target, review process, staging environments, compliance needs, current pain points with deploys..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!stack||!requirements} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Designing...':'Design Pipeline'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_productgrowth188() {
  const [product, setProduct] = React.useState('');
  const [constraint, setConstraint] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/product/growth-framework', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,constraint})}); const d = await r.json(); setResult(d.framework||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📐 Growth Framework Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build a growth framework that finds your real lever — the constraint holding back your growth and the systematic approach to removing it.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={product} onChange={e=>setProduct(e.target.value)} placeholder="Product: current metrics (DAU, MAU, retention, activation rate, conversion), growth channels, what is working and not..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={constraint} onChange={e=>setConstraint(e.target.value)} placeholder="Biggest constraint (e.g. poor activation, high early churn, weak top-of-funnel, low expansion revenue)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!product||!constraint} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Growth Framework'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_salesenablement189() {
  const [product, setProduct] = React.useState('');
  const [objections, setObjections] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/sales/enablement-content', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({product,objections})}); const d = await r.json(); setResult(d.content||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🎓 Sales Enablement Content</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Create sales enablement content that actually helps reps close — not slide decks they ignore, but the one-pagers, battle cards, and talk tracks they reach for every day.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={product} onChange={e=>setProduct(e.target.value)} placeholder="Product: what it does, key differentiators, target personas, typical deal size and cycle, how reps currently sell..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={objections} onChange={e=>setObjections(e.target.value)} placeholder="Top objections reps face: price, competitor, timing, DIY, no budget, need approval — list the ones that kill the most deals..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!product||!objections} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Creating...':'Create Content'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_devmonitor189() {
  const [system, setSystem] = React.useState('');
  const [sla, setSla] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/dev/monitoring-setup', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({system,sla})}); const d = await r.json(); setResult(d.setup||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📡 Monitoring Setup Guide</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Design your observability stack — the metrics, logs, traces, and alerts that catch problems before users do, with thresholds that page you for the right reasons.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={system} onChange={e=>setSystem(e.target.value)} placeholder="System: architecture (services, database, queue, cache), traffic patterns, current monitoring if any, biggest reliability concerns..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={sla} onChange={e=>setSla(e.target.value)} placeholder="SLA targets (e.g. 99.9% uptime, p99 < 300ms, error rate < 0.1%)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!system||!sla} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Designing...':'Design Monitoring'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_contentcalendar189() {
  const [goals, setGoals] = React.useState('');
  const [channels, setChannels] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/marketing/content-calendar', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({goals,channels})}); const d = await r.json(); setResult(d.calendar||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📅 Content Calendar Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build a 30-day content calendar that is actually achievable — realistic cadence, content pillars, and a production plan your team can execute without burning out.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={goals} onChange={e=>setGoals(e.target.value)} placeholder="Goals: what content is trying to achieve (leads, brand, SEO, retention, pipeline), target audience, themes to hit this month..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={channels} onChange={e=>setChannels(e.target.value)} placeholder="Channels (e.g. LinkedIn 3x/week, blog 2x/month, email newsletter weekly, Twitter daily)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!goals||!channels} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Calendar'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_investormodel189() {
  const [business, setBusiness] = React.useState('');
  const [assumptions, setAssumptions] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/investor/financial-model-guide', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({business,assumptions})}); const d = await r.json(); setResult(d.guide||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📊 Financial Model Guide</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build a financial model that investors trust — the right structure, the right assumptions, and the scenarios that show you understand your business deeply.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={business} onChange={e=>setBusiness(e.target.value)} placeholder="Business: revenue model, pricing, current metrics (ARR, growth, burn), headcount plan, major cost drivers..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={assumptions} onChange={e=>setAssumptions(e.target.value)} placeholder="Key assumptions: growth rate, churn, CAC, conversion rates, hiring plan, what the model needs to show (path to profitability, runway, TAM capture)..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!business||!assumptions} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Financial Model Guide'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_teamstructure189() {
  const [company, setCompany] = React.useState('');
  const [goals, setGoals] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/hr/org-design', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({company,goals})}); const d = await r.json(); setResult(d.design||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🏗️ Org Design Advisor</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Design your org structure for the next phase — team shapes, reporting lines, span of control, and the trade-offs between speed and coordination at your scale.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={company} onChange={e=>setCompany(e.target.value)} placeholder="Company: current headcount, structure, what is working or breaking, leadership team, products/customers..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={goals} onChange={e=>setGoals(e.target.value)} placeholder="Goals: what you are optimizing for (speed, accountability, customer success, platform quality), pain points in current org, planned headcount in 12 months..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!company||!goals} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Designing...':'Design Org Structure'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_devcontainerize190() {
  const [app, setApp] = React.useState('');
  const [infra, setInfra] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/dev/containerization-guide', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({app,infra})}); const d = await r.json(); setResult(d.guide||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🐳 Containerization Guide</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Containerize your application the right way — Dockerfile best practices, orchestration options, and the patterns that keep images small, builds fast, and production stable.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={app} onChange={e=>setApp(e.target.value)} placeholder="Application: language/runtime, build process, services it depends on (DB, cache, queue), environment variables, startup time considerations..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={infra} onChange={e=>setInfra(e.target.value)} placeholder="Target infra (e.g. Kubernetes on GKE, ECS on AWS, Railway, Fly.io, Docker Compose for local)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!app||!infra} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Guide'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_productabtest190() {
  const [hypothesis, setHypothesis] = React.useState('');
  const [context, setContext] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/product/ab-test-design', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({hypothesis,context})}); const d = await r.json(); setResult(d.design||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🧪 A/B Test Designer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Design a rigorous A/B test — sample size, duration, metric selection, and the guardrails that prevent you from shipping a change that hurts the metrics you did not measure.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={hypothesis} onChange={e=>setHypothesis(e.target.value)} placeholder="Hypothesis: if we [change], then [metric] will [increase/decrease] by [amount], because [reason]..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={context} onChange={e=>setContext(e.target.value)} placeholder="Context: current metric baseline, traffic volume per day/week, what the change is, what other metrics to watch, testing platform..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!hypothesis||!context} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Designing...':'Design A/B Test'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_marketingroi190() {
  const [spend, setSpend] = React.useState('');
  const [goals, setGoals] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/marketing/roi-calculator', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({spend,goals})}); const d = await r.json(); setResult(d.roi||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>💡 Marketing ROI Calculator</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Calculate and communicate your marketing ROI — channel-by-channel analysis, attribution approach, and the reporting framework that gets marketing a seat at the revenue table.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={spend} onChange={e=>setSpend(e.target.value)} placeholder="Marketing spend: breakdown by channel and amount (e.g. Google Ads $10K, content $5K, events $8K, tools $2K), time period..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={goals} onChange={e=>setGoals(e.target.value)} placeholder="Results: leads generated, deals influenced, pipeline created, revenue closed, how you currently track attribution..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!spend||!goals} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Calculating...':'Calculate ROI'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_salesprocess190() {
  const [current, setCurrent] = React.useState('');
  const [product, setProduct] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/sales/process-design', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({current,product})}); const d = await r.json(); setResult(d.process||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🔄 Sales Process Designer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Design a repeatable sales process — the stages, exit criteria, activities, and handoffs that let you scale from founder sales to a full team without losing win rate.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={current} onChange={e=>setCurrent(e.target.value)} placeholder="Current process: how you sell today, average cycle length, where deals stall or die, conversion rates at each stage if known..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={product} onChange={e=>setProduct(e.target.value)} placeholder="Product and deal context: deal size, buyer profile, number of stakeholders in deal, technical evaluation required, procurement involved..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!current||!product} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Designing...':'Design Sales Process'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_devapi190() {
  const [service, setService] = React.useState('');
  const [consumers, setConsumers] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/dev/api-strategy', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({service,consumers})}); const d = await r.json(); setResult(d.strategy||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🔌 API Strategy Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Design an API strategy that serves developers well — the right paradigm, versioning approach, auth, rate limiting, and documentation that makes your API a competitive advantage.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={service} onChange={e=>setService(e.target.value)} placeholder="Service: what capabilities you are exposing, data types, operations needed, scale expectations, existing API if any..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={consumers} onChange={e=>setConsumers(e.target.value)} placeholder="Consumers: who will use this (internal teams, external developers, partners, mobile apps), their technical sophistication, integration patterns..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!service||!consumers} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build API Strategy'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_fundraisetimeline191() {
  const [stage, setStage] = React.useState('');
  const [metrics, setMetrics] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/investor/fundraise-timeline', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({stage,metrics})}); const d = await r.json(); setResult(d.timeline||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🗓️ Fundraise Timeline Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Plan your fundraise from first LP outreach to close — the sequencing, the milestones to hit before you start, and the process management that keeps you in control of your timeline.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={stage} onChange={e=>setStage(e.target.value)} placeholder="Round: size, type (seed, Series A, bridge), investors you are targeting, past investors, any lead committed..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={metrics} onChange={e=>setMetrics(e.target.value)} placeholder="Current metrics: ARR, growth, burn, runway, team, what you are trying to show before you raise, target close date..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!stage||!metrics} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Planning...':'Build Timeline'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_devobservability191() {
  const [services, setServices] = React.useState('');
  const [incidents, setIncidents] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/dev/observability-audit', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({services,incidents})}); const d = await r.json(); setResult(d.audit||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🔭 Observability Audit</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Audit your observability gaps — find the blind spots that made recent incidents harder to diagnose and the instrumentation improvements that will cut your MTTR in half.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={services} onChange={e=>setServices(e.target.value)} placeholder="Services: what you run (APIs, workers, databases, queues), current monitoring tools, what you can see vs. cannot see today..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={incidents} onChange={e=>setIncidents(e.target.value)} placeholder="Recent incidents: what happened, how long to detect, how long to diagnose, what information you wished you had had..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!services||!incidents} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Auditing...':'Run Audit'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_productpricing191() {
  const [segment, setSegment] = React.useState('');
  const [value, setValue] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/product/willingness-to-pay', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({segment,value})}); const d = await r.json(); setResult(d.analysis||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>💰 Willingness-to-Pay Analyzer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Analyze what customers will actually pay — van Westendorp method, conjoint analysis principles, and the signals from your own sales data that reveal true price elasticity.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={segment} onChange={e=>setSegment(e.target.value)} placeholder="Customer segment: company size, industry, role of buyer, annual budget they control, how they justify software spend..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={value} onChange={e=>setValue(e.target.value)} placeholder="Value delivered: what your product replaces or enables, hours saved, revenue generated, risk reduced, deals won or lost on price..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!segment||!value} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Analyzing...':'Analyze WTP'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_marketingpersona191() {
  const [customers, setCustomers] = React.useState('');
  const [data, setData] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/marketing/buyer-persona', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({customers,data})}); const d = await r.json(); setResult(d.personas||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>👤 Buyer Persona Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build buyer personas grounded in real data — not demographic guesses, but the goals, fears, buying triggers, and language patterns that let you write copy that makes prospects feel understood.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={customers} onChange={e=>setCustomers(e.target.value)} placeholder="Customer data: who buys from you today, company profile, titles involved in purchase, deal sizes, industries, what drove them to you..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={data} onChange={e=>setData(e.target.value)} placeholder="Research data: common objections you hear, words customers use to describe their problem, what they tried before, what they care about most..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!customers||!data} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Personas'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_hrcompensation191() {
  const [role, setRole] = React.useState('');
  const [market, setMarket] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/hr/comp-benchmarking', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({role,market})}); const d = await r.json(); setResult(d.benchmark||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📏 Comp Benchmarking Guide</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Benchmark compensation for any role — market rates by level, location, and company stage, with a framework for building a comp philosophy that attracts talent without blowing your budget.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={role} onChange={e=>setRole(e.target.value)} placeholder="Role: title, level (IC3/IC4/Staff, junior/mid/senior), function, key responsibilities, tech stack if engineering..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={market} onChange={e=>setMarket(e.target.value)} placeholder="Market context: company stage, location (SF/NYC/remote), funding level, how comp-competitive you want to be (50th/75th/90th percentile)..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!role||!market} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Benchmarking...':'Benchmark Comp'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_devdataengineering192() {
  const [pipeline, setPipeline] = React.useState('');
  const [scale, setScale] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/dev/data-pipeline-design', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({pipeline,scale})}); const d = await r.json(); setResult(d.design||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🔧 Data Pipeline Designer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Design a data pipeline that is reliable, testable, and does not become someone's full-time job to maintain — the right architecture for your scale and team size.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={pipeline} onChange={e=>setPipeline(e.target.value)} placeholder="Pipeline: what data you need to move (sources, destinations, transformations), latency requirements (real-time, near-real-time, batch)..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={scale} onChange={e=>setScale(e.target.value)} placeholder="Scale (e.g. 100K events/day, 10M rows/month, team of 2 data engineers)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!pipeline||!scale} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Designing...':'Design Pipeline'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_productfeatureflags192() {
  const [team, setTeam] = React.useState('');
  const [usecase, setUsecase] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/product/feature-flag-strategy', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({team,usecase})}); const d = await r.json(); setResult(d.strategy||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🚩 Feature Flag Strategy</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build a feature flag strategy that enables progressive delivery without turning your codebase into a flag graveyard — the governance, naming, and lifecycle management that scales.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={team} onChange={e=>setTeam(e.target.value)} placeholder="Team: size, deploy frequency, current flag tool if any, what causes you the most deployment anxiety today..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={usecase} onChange={e=>setUsecase(e.target.value)} placeholder="Primary use case (e.g. gradual rollouts, A/B testing, kill switches, beta programs, per-customer features)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!team||!usecase} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Strategy'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_saleschampion192() {
  const [deal, setDeal] = React.useState('');
  const [champion, setChampion] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/sales/champion-builder', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({deal,champion})}); const d = await r.json(); setResult(d.strategy||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>🏆 Champion Builder</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build and arm your internal champion — the person who sells for you when you are not in the room, with the materials and talking points that help them win the internal budget conversation.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={deal} onChange={e=>setDeal(e.target.value)} placeholder="Deal: company, size, what they are evaluating, stakeholders involved, decision timeline, key objections or political dynamics..." rows={3} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={champion} onChange={e=>setChampion(e.target.value)} placeholder="Champion profile (title, influence level, their personal stake in this decision, what success looks like for them)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!deal||!champion} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Building...':'Build Champion Strategy'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_marketingcopyreview192() {
  const [copy, setCopy] = React.useState('');
  const [audience, setAudience] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/marketing/copy-review', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({copy,audience})}); const d = await r.json(); setResult(d.review||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>✍️ Marketing Copy Reviewer</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Get an expert review of your marketing copy — the clarity issues, the weak CTAs, the jargon that confuses buyers, and the specific rewrites that will convert better.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={copy} onChange={e=>setCopy(e.target.value)} placeholder="Copy to review: paste your landing page, ad, email, or headline — include as much context as you have..." rows={5} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><input value={audience} onChange={e=>setAudience(e.target.value)} placeholder="Target audience and goal (e.g. VP Sales at 50-500 person SaaS, goal is demo signup)" style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem'}} /><button onClick={run} disabled={loading||!copy||!audience} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Reviewing...':'Review Copy'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}

export function ForgeTab_productpriorityframework192() {
  const [backlog, setBacklog] = React.useState('');
  const [context, setContext] = React.useState('');
  const [result, setResult] = React.useState('');
  const [loading, setLoading] = React.useState(false);
  const API = (window as any).FORGE_API || '';
  const run = async () => { setLoading(true); setResult(''); try { const r = await fetch(API+'/api/product/prioritization-framework', {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('forge_token')||'')},body:JSON.stringify({backlog,context})}); const d = await r.json(); setResult(d.framework||d.error||JSON.stringify(d)); } catch(e:any){setResult(e.message);} finally{setLoading(false);} };
  return (<div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}><h2>📋 Prioritization Framework</h2><p style={{color:'#888',marginBottom:'1.5rem'}}>Build a prioritization framework your team will actually use — not a spreadsheet that gathers dust, but a clear decision process that cuts through stakeholder noise and focuses on what moves the needle.</p><div style={{display:'grid',gap:'1rem'}}><textarea value={backlog} onChange={e=>setBacklog(e.target.value)} placeholder="Backlog items: list the features or initiatives you need to prioritize (at least 5-10 items with brief description of each)..." rows={4} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><textarea value={context} onChange={e=>setContext(e.target.value)} placeholder="Context: current goals (OKRs, revenue target, retention fix), stakeholder pressures, team capacity, what the board or CEO is pushing..." rows={2} style={{width:'100%',background:'#1a1a2e',color:'#e0e0e0',border:'1px solid #333',borderRadius:8,padding:'0.75rem',resize:'vertical'}} /><button onClick={run} disabled={loading||!backlog||!context} style={{padding:'0.75rem 2rem',background:'#6c63ff',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',opacity:loading?0.6:1}}>{loading?'Prioritizing...':'Build Framework'}</button></div>{result&&<div style={{marginTop:'1.5rem',background:'#1a1a2e',padding:'1.5rem',borderRadius:8,border:'1px solid #333',whiteSpace:'pre-wrap',lineHeight:1.7}}>{result}</div>}</div>);
}
