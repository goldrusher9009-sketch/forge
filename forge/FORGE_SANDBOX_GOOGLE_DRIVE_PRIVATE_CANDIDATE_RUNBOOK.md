...
The candidate VPS now provides two local systemd controls:

- `forge-candidate-backup.timer` runs a daily backup in the randomized window beginning at 03:15 UTC. It uses `better-sqlite3.backup()` for an application-consistent snapshot, compresses the snapshot, restores it in a network-disabled container with a read-only root filesystem, requires `integrity_check=ok` and matching key-table counts, writes a SHA-256 sidecar, and leaves both files as `root:root 0600`. It does not delete prior backups automatically.
- `forge-candidate-monitor.timer` runs every five minutes and verifies the immutable release marker; Forge, Orchestrator, and internal gateway container state, health, and restart count; Forge and Orchestrator HTTP readiness; Nginx state; newest backup age and SHA-256; and host free space. It writes to the local systemd journal only. It does not provide external paging, webhook delivery, escalation, or on-call ownership.

These controls improve private-candidate recovery and detection on the same VPS. They are not off-host backup, geographic disaster recovery, centralized monitoring, or an external alerting service.
...
- Orchestrator unit tests: 13/13 passed;
...
- candidate inbound-content policy: verified on the candidate branch and current candidate VPS. Browser Workspace uploads and Google Drive imports converge on the Orchestrator file-ingress boundary. Policy `forge-inbound-v2` retains executable/installer/disk-image/macro-enabled extension and renamed native-signature blocking, then performs bounded ZIP/Office-container inspection before any Workspace volume helper is created. It rejects malformed or encrypted archives, multipart/ZIP64 or unsupported compression, local/central filename mismatches, path traversal and symbolic links, nested executable or uninspected archive entries, VBA/ActiveX/OLE internals, excessive entry or expanded-byte totals, and suspicious compression ratios. Normal documents, source files, PDFs, structurally valid DOCX/XLSX/PPTX, and inspectable ZIP archives remain supported; opaque or nested archives fail closed. The isolated Google Drive regression still proves that a rejected import is returned as HTTP 422, records a `failed` transfer ledger row with the policy error, and creates no successful Workspace write. Real Orchestrator E2E proved a traversal ZIP returned `SANDBOX_UPLOAD_CONTENT_BLOCKED_ARCHIVE_PATH_UNSAFE`, a safe ZIP and ordinary upload both carried `contentPolicy=forge-inbound-v2`, public egress and private-address controls still passed, and cleanup returned managed containers and volumes to zero. A live Forge API acceptance propagated the archive policy error as HTTP 422 without creating a managed container or volume;
...
- automated SQLite backup and local-monitor acceptance: the VPS systemd service now uses `better-sqlite3.backup()` before gzip, restores every newly created snapshot in a network-disabled/read-only container, requires pure single-line JSON evidence with `integrity_check=ok` and matching key-table counts, verifies SHA-256, and writes backup plus sidecar as `root:root 0600`. A manual execution and the hardened systemd execution both passed. The daily backup timer and five-minute local readiness timer are active and enabled; the latest backup and monitor invocations completed with status 0, zero restarts, and no Docker-config permission warning;
...
- current VPS operations release `8af22c62ff7f8bb1e2457d647faf30eaffbb5299`: the immutable release switch added the verified backup/monitor scripts and systemd units without rebuilding or recreating Forge, Orchestrator, Sandbox Docker, the internal gateway, ProjectHash, or Nginx. All four Forge containers remain healthy with zero restarts, ProjectHash and Nginx retained their exact identities, and managed per-Run containers and volumes remain at zero. The previously deployed v2 archive-policy runtime and its post-switch concurrency evidence remain unchanged;
...
- approved production DNS, TLS, reverse proxy, firewall, host, and secret store; automated off-host/geographically independent backup retention and restore drills; centralized monitoring; external alert delivery and escalation; and on-call ownership. The current VPS-local automatic backups and five-minute local readiness checks are verified and usable for candidate recovery/detection, but they are not geographic disaster recovery or an externally delivered alerting/on-call system;
...
- full signature-based malware scanning, recursive decompression/content scanning of nested or opaque archives, DLP, and organization-specific content policy remain open. Candidate policy `forge-inbound-v2` performs bounded structural ZIP/Office inspection and blocks uninspectable archive forms at the shared Workspace/Drive ingress boundary, but it must not be described as antivirus, recursive malware scanning, or comprehensive DLP;
