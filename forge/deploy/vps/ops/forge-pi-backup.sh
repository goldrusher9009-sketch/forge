...
PLATFORM_CONTAINER="${FORGE_PLATFORM_CONTAINER:-forge-pi-forge-platform-1}"
LOCK_FILE="${FORGE_BACKUP_LOCK_FILE:-/run/lock/forge-pi-backup.lock}"
DOCKER_CONFIG_DIR=/run/forge-pi-backup

fail() {
  printf 'forge-pi backup: FAIL %s\n' "$1" >&2
  exit 1
}
...
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  printf 'forge-pi backup: SKIP another verified backup is running\n'
  exit 0
fi
...
printf 'forge-pi backup: PASS file=%s bytes=%s sha256=%s evidence=%s\n' \
