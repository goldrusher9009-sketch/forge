...
PLATFORM_CONTAINER="forge-pi-forge-platform-1"
ORCHESTRATOR_CONTAINER="forge-pi-forge-sandbox-orchestrator-1"
GATEWAY_CONTAINER="forge-pi-forge-control-plane-gateway-1"
WORKER_CONTAINER="forge-pi-forge-pi-worker-1"
DOCKER_CONFIG_DIR=/run/forge-pi-monitor
failures=()
...
emit_failure() {
  ...
  if command -v logger >/dev/null 2>&1; then
    logger -t forge-pi-monitor -- "FAIL $message"
  fi
  printf 'forge-pi monitor: FAIL %s\n' "$message" >&2
  exit 1
}
...
# forge-pi is synced as a plain directory; there is no release symlink to verify.
...
logger -t forge-pi-monitor -- "PASS stack=forge-pi backup=$(basename "$latest_backup") free_bytes=$free_bytes"
printf 'forge-pi monitor: PASS stack=forge-pi backup=%s free_bytes=%s\n' "$(basename "$latest_backup")" "$free_bytes"
